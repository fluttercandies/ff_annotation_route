...
class PermissionInterceptor extends IPermissionInterceptor {
  ...
  Future<RouteInterceptResult> intercept(
    ...
    if (!User().isAmdin) {
      showDialog(
        ...
        builder: (b) {
          return AlertDialog(
            ...
            content: Text(
              'You do not have permission to access $routeName page.',
            ),
            actions: [
              TextButton(
                onPressed: () {
      ...
      return RouteInterceptResult.abort(routeName: routeName);
    }

    return RouteInterceptResult.next(routeName: routeName);
  }
}
...
class GlobalPermissionInterceptor extends RouteInterceptor {
  ...
  Future<RouteInterceptResult> intercept(
    ...
    if (routeName == Routes.pageB.name) {
      if (!User().isAmdin) {
        showDialog(
          context: GlobalNavigator.context!,
          builder: (b) {
